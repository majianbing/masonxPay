...
/**
 * 3DS Return Page (subscription checkout)
 *
 * Stripe redirects here after a 3DS challenge completes (both success and failure), as part
 * of the INCOMPLETE-subscription first-charge flow. This page is opened inside the SDK's
 * iframe overlay, so it must send a postMessage to the parent window then show a neutral
 * "completing payment" spinner.
 *
 ...
 *   subscriptionToken             — subscription checkout link token so the parent can poll
 *
...
function ThreeDsReturnInner() {
  ...
  useEffect(() => {
    ...
    const subscriptionToken          = searchParams.get('subscriptionToken') ?? undefined;
    ...
    try {
      window.parent.postMessage(
        { type: 'gw:3ds_complete', subscriptionToken, redirectStatus, paymentIntentClientSecret },
        '*',
      );
    } catch {
