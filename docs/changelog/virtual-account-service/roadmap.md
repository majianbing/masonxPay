...
- **Ledger review hardening — fixes #1–#3** (2026-07-19): see `docs/refactor/ledger-review-hardening.md`. (1) Idempotent card authorization: `card_authorization` decision log (V10), UNIQUE(issuer_id, authorization_id), replay on duplicate delivery, fail-closed APPROVED⟹hold invariant, issuer-agnostic `CardAuthorizationService` with ISO 8583 vocabulary confined to rail-simulator; card identity by `card_token_id` (V11). (2) Settlement exception parking: `settlement_exception` table (V12), handlers park unpostable events, Kafka DefaultErrorHandler backstop with parking recoverer, ops retry/discard API, `va_settlement_exceptions_open` gauge. (3) Platform-books convention: WALLET/PREPAID_CARD/PREPAID_CARD_HOLD flipped to CREDIT-normal liabilities, card posting rule legs flipped, `MERCHANT_RECEIVABLE` (V13/V14) books bank-return shortfalls with recoupment from later settlements, `PLATFORM_FEE_RECEIVABLE` (V15) accrues gateway fees. V7 seed history rewritten — dev DB reset required. Runtime-verified on the bench stack: replay-storm (500 concurrent deliveries → one hold), park→retry→resolve loop, settle→shortfall-return→recoupment money story, trial balance held. 109 VA unit tests.
