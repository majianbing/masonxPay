...
- Manual dashboard testing then found the code-review fix's new idempotency-lookup queries broke under this system's ShardingSphere sharding layer (Spring Data's `StartingWith` keyword generates a `LIKE ... ESCAPE` clause ShardingSphere can't parse); rewritten as explicit `@Query`s and re-verified live (see Changelog). Neither defect was caught by the unit suite — the test profile doesn't route through ShardingSphere — so treat this module's sharded-table query changes as needing a live smoke test, not just `mvn test`, before merge.
...
- 2026-07-01: Code-review fix. `prepareStripe()` still sent Stripe a fresh `UUID.randomUUID()`-derived idempotency key on every call, with no claim guard and no DB state check — contrary to the Phase 8 addendum note above, this redirect-prep call site was never actually made idempotent. Replaced with a deterministic per-attempt key (`pl-{linkId}-stripe-pi-{attemptNumber}`) plus an idempotent DB lookup (`PaymentIntentRepository.findByMerchantIdAndIdempotencyKeyStartingWithOrderByCreatedAtDesc` / `countByMerchantIdAndIdempotencyKeyStartingWith`) that resumes an in-flight attempt instead of minting a duplicate Stripe PaymentIntent; concurrent-request races are resolved via the existing `(merchant_id, idempotency_key)` unique constraint. Added `PublicPaymentControllerTest` coverage for the branch that doesn't require mocking the Stripe SDK's static calls (this module pins Mockito to `mock-maker-subclass`, so the Stripe-calling branches remain uncovered by unit tests, same as before this fix).
- 2026-07-02: Manual-testing fix. The idempotency-lookup queries added in the fix above used Spring Data's `StartingWith` derived-query keyword, which auto-appends `LIKE ? ESCAPE '\''`. The ShardingSphere SQL router in front of the logically sharded `payment_intents` table cannot parse that construct ("no viable alternative at input ... order"), so every `prepare-stripe` call 500'd — caught by manual dashboard testing (connector preview / hosted checkout flow) before merge, not by the unit suite, since the test profile doesn't route through ShardingSphere. Rewrote both queries as explicit `@Query`s using `LIKE CONCAT(:prefix, '%')` with no `ESCAPE` clause (safe: the prefix is always built from UUIDs/literal text, never user input). Verified against the running dev DB by inserting a temporary payment link: `prepare-stripe` now returns 200, and calling it twice for the same link returns the same Stripe client secret with exactly one local `PaymentIntent` row created.
...
- 2026-07-02: Dashboard payment-list fix. Subscription-created payment intents already received `pi_...` external IDs, but the projection path could still show UUIDs because invoice/subscription checkout flows wrote only billing/subscription events that the payment projection ignores, and the startup read-model backfill did not insert/update `payment_read_models.external_id`. Added `V71__backfill_payment_read_model_external_id.sql`, updated the projection backfill SQL to carry/search `external_id`, and now emits payment-intent lifecycle outbox events from invoice billing and subscription checkout/resume/cancel paths so dashboard payment lists and overview rows can display `externalId` instead of UUID.
