...
Status: **Complete — all 5 steps done.** Branch: `refactor/modularize-backend`.
...
| 5 | Docker: keep minimum stack; add `virtual-account` profile (VA + Kafka + `msx_virtual_account` DB); update `backend/Dockerfile` (reactor build, jar paths) and compose. | Minimum stack boots without VA; profile boots VA end-to-end. | ☑ (both images build; compose validates base + profile; CI fixed) |
...
- **Step 4 done** (scope: bootable skeleton + plumbing, by decision) — added `virtual-account-service`: bootable `VirtualAccountApplication`, own DataSource → `msx_virtual_account` (single DataSource, no ShardingSphere), Flyway `db/migration/va/V1__baseline.sql` (inbox/idempotency table only). Inbound seam: `SettlementEventConsumer` (`@KafkaListener`, autoStartup gated by `va.kafka.consumer.enabled`) → ACL `SettlementEventMapper` (only place importing `contracts`) → VA-native `RecordSettlementCommand` → `SettlementHandler`/`LoggingSettlementHandler` stub; `InboxRepository` does DB-level dedup (`ON CONFLICT DO NOTHING`). Wired into reactor; `backend/Dockerfile` builds gateway via `-pl gateway-service -am` (VA pom present, not baked into the gateway image). `mvn -DskipTests package` green (5 modules, VA boot jar produced); `docker compose build backend` green. **No ledger/account tables** — owned by the VA domain design. VA standalone boot + idempotent consume need `msx_virtual_account` + Kafka (not run here).
  - **Note (new red line):** VA business code will use snowflake IDs with a business prefix (e.g. `ac_{snowflakeId}`), not UUID. The skeleton mints no VA business-entity IDs (ledger deferred); UUIDs present are gateway's existing identifiers crossing the boundary (`paymentId`/`merchantId`/`orgId`, grandfathered).
- **Snowflake event ids** — adopted the new rule for event ids: `EventEnvelope.eventId` is now `String` (`evt_{snowflakeId}`, producer-minted), with VA's inbox PK and `RecordSettlementCommand.sourceEventId` following. Gateway-sourced ids stay UUID (grandfathered). Snowflake generator deferred until a producer mints events. Compile green.
- **Step 5 done** — Docker profile + CI. Creates `msx_virtual_account` via an inline compose `configs:` block (`va_init_db`) mounted to the postgres service's `/docker-entrypoint-initdb.d` — kept in `docker-compose.yml` itself (no external script), since `POSTGRES_DB` only creates one database. Added `backend/Dockerfile.va` (builds VA via `-pl virtual-account-service -am`). Added the optional `virtual-account` compose service (profile `virtual-account`, port 8086, depends on postgres + kafka); added `virtual-account` to Kafka's profiles so the profile activates it. **Minimum stack unchanged** (`backend + dashboard + pg`); VA/Kafka are opt-in. CI: fixed `.github/workflows/maven.yml` (JDK 17→21, `--file backend/pom.xml`, `-DskipTests` since tests need external infra) and removed the irrelevant `gradle-publish.yml`. Verified: `docker compose config` valid for base **and** `--profile virtual-account`; `docker compose build backend` and `docker compose --profile virtual-account build virtual-account` both green.
  - **Operational note:** the `msx_virtual_account` init config runs only on a **fresh** Postgres volume. On an existing volume the DB won't exist (VA Flyway fails with `database "msx_virtual_account" does not exist`); create it once without data loss:
    `docker compose exec -T postgres psql -U "${DB_USERNAME:-pay_app_user}" -d "${DB_NAME:-paygateway}" -c "CREATE DATABASE msx_virtual_account;"`
  - **Profiles:** `virtual-account` is a member of **both** `infra` and `virtual-account` profiles — `--profile infra` brings up everything optional (kafka + redis + VA) for local testing; `--profile virtual-account` brings up just kafka + VA. Verified live: VA boots, Flyway applies `V1__baseline`, Tomcat on 8086, consumer subscribes to `payment.settlement.events`.
  - **CI follow-up:** running the test suite in CI needs Postgres (+ Kafka) service containers; currently CI builds/packages only.

## Outcome

All five steps complete. The backend is now a Maven reactor — `common`, `contracts`, `gateway-service` (bootable, unchanged internals), `virtual-account-service` (bootable, optional, own `msx_virtual_account` DB, Kafka-consumer seam with ACL). Gateway behavior is unchanged; VA is independently deployable and microservice-ready. Remaining work is the VA **domain** (ledger/money/snowflake ids) — owned per `virtual-account-guide.md` — and wiring gateway to actually **produce** the settlement event (with the money payload) when that domain is designed.
