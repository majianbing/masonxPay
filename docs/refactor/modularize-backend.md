...
- **Step 5 done** — Docker profile + CI. Creates `msx_virtual_account` via an inline compose `configs:` block (`va_init_db`) mounted to the postgres service's `/docker-entrypoint-initdb.d` — kept in `docker-compose.yml` itself (no external script), since `POSTGRES_DB` only creates one database. Added `backend/Dockerfile.va` (builds VA via `-pl virtual-account-service -am`). Added the optional `virtual-account` compose service (profile `virtual-account`, port 8081, depends on postgres + kafka); added `virtual-account` to Kafka's profiles so the profile activates it. **Minimum stack unchanged** (`backend + dashboard + pg`); VA/Kafka are opt-in. CI: fixed `.github/workflows/maven.yml` (JDK 17→21, `--file backend/pom.xml`, `-DskipTests` since tests need external infra) and removed the irrelevant `gradle-publish.yml`. Verified: `docker compose config` valid for base **and** `--profile virtual-account`; `docker compose build backend` and `docker compose --profile virtual-account build virtual-account` both green.
  - **Operational note:** the `msx_virtual_account` init script runs only on a fresh Postgres volume. Existing volumes need a manual `CREATE DATABASE msx_virtual_account;` (or a volume reset) before enabling the VA profile.
