...
| S1 | **Customer and payment method foundation** | [~] | Added merchant-scoped, TEST/LIVE-isolated customers, customer payment-method references backed by safe `PaymentInstrument` rows, backend APIs, focused service tests, and dashboard customer management. Broader API/controller coverage remains pending. |
| S2 | **Subscription and invoice foundation** | [~] | Added mode-aware subscription records, subscription items, trial-aware creation, merchant checkout-link tokens, backend APIs, public checkout-link lookup, TEST-mode simulator activation, invoice storage, idempotent current-period invoice generation, and Docker Compose verification. Real PSP reusable-method setup, promotions/coupons, automated period advancement, and recurring invoice generation remain pending. |
| S3 | **Off-session invoice payment execution** | [ ] | Pay open invoices through normal payment intent/provider routing using saved reusable PSP references and Mason Simulator tests. Requires customer-present setup or first-payment authorization before monthly deductions. |
