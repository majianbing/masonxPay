...
| S4 | **Recurring retry and dunning** | ✅ | Reused existing `scheduled_retry_jobs` infrastructure — added `invoice_id` column (V59), `INVOICE_PAYMENT` operation, and extended `ScheduledRetryWorkerService`. Failed invoice payment auto-schedules a retry job (3 attempts, 3/5/7-day delays, configurable via payload_json). Hard declines do not retry. Final actions on exhaustion: PAST_DUE (default), CANCEL subscription, or UNCOLLECTIBLE invoice. |
