...
| S2 | **Subscription and invoice foundation** | [~] | Added mode-aware subscription records, subscription items, trial-aware creation, merchant checkout-link tokens, backend APIs, public checkout-link lookup, TEST-mode simulator activation, invoice storage, idempotent current-period invoice generation, Docker Compose verification, and reusable payment-method setup boundary for Stripe/Square/Braintree/Simulator. Mollie mandate completion, promotions/coupons, automated period advancement, and recurring invoice generation remain pending. |
