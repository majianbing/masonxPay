...
| S1 | **Customer and payment method foundation** | [~] | Added merchant-scoped customers, customer payment-method references backed by safe `PaymentInstrument` rows, backend APIs, focused service tests, and dashboard customer management. Broader API/controller coverage remains pending. |
| S2 | **Subscription and invoice foundation** | [~] | Added mode-aware subscription records, subscription items, trial-aware creation, merchant checkout-link tokens, backend APIs, public checkout-link lookup, TEST-mode simulator activation, and Docker Compose verification. Reusable promotions/coupons, invoices, period generation, and idempotent invoice creation remain pending. |
| S3 | **Off-session invoice payment execution** | [ ] | Pay open invoices through normal payment intent/provider routing using saved instruments and Mason Simulator tests. |
