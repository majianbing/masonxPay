...
- Citation accuracy.
- Retrieval recall@k, MRR, and nDCG against relevance-judged questions.
- Generated-answer groundedness and faithfulness when model generation is enabled.
...
- There is no trained semantic embedding model. `JsonRetriever` ranks lexical token overlap, while Qdrant and the experimental LlamaIndex backend use deterministic 384-dimensional signed token-hash vectors.
- Chunking is a fixed heading-aware split with a 1,600-character limit. It has not been tuned against retrieval metrics.
- The seven-question golden suite checks refusals, required/forbidden citation paths, audience filtering, and required answer substrings. It does not measure ranking quality, retrieval recall, or generated-answer groundedness.
- The dashboard is single-turn and has no answer-feedback controls or feedback persistence.
- Optional external generation changes answer synthesis only. It does not improve embeddings, retrieval, or ranking.

Current execution order:

1. Continue RAG7 production hardening, especially authenticated/private Qdrant deployment and operational controls.
2. Establish the RAG8 semantic retrieval and measurement baseline.
3. Resume RAG5 framework comparison only after RAG8 provides discriminating data and metrics.
...
| RAG2 | **Bootstrap ingestion and indexing** | [x] | Complete for bootstrap: approved docs are chunked with stable chunk IDs, source hashes, git commit metadata, index timestamps, deterministic token-hash vectors, stable Qdrant point IDs, stale-vector cleanup, manual freshness checks, and API/UI surfacing of indexed source version. Semantic embedding and retrieval quality belong to RAG8. |
...
| RAG4 | **Dashboard assistant UI and feedback** | [~] | In progress: the read-only dashboard assistant page, index metadata display, citations, unsupported-answer states, copyable source paths, source metadata badges, and dashboard production build verification are complete. Answer feedback controls and feedback persistence are not implemented. |
...
| RAG6 | **Governance evals and auditability** | [x] | Complete for bootstrap: unit tests cover citations, sensitive-data refusal, audience filtering, deterministic token-hash vectors, gateway facade behavior, compose config, and stable-doc-over-planning conflict handling. The golden-question runner is a governance regression gate, not a retrieval-ranking benchmark. Answers/status include prompt-template, answer-policy, model-provider, and model-name metadata. AI service audit records are safe JSONL records with hashed questions, citation paths, refusal/confidence, retrieved chunk IDs, and version metadata. Retrieval metrics and groundedness evaluation belong to RAG8. |
| RAG7 | **Production hardening** | [~] | In progress: gateway-to-AI bearer authentication is implemented with gateway `AI_SERVICE_AUTH_TOKEN` mapped to ai-service `RAG_AUTH_TOKEN` plus `RAG_REQUIRE_AUTH=true`; local extractive generation is the no-external-model default; bootstrap deployments still warn when auth is not required or Qdrant uses plain HTTP. Remaining: vector DB auth/TLS where supported, broader provider fallback/deployment controls, alerting, and operational runbooks. |
| RAG8 | **Semantic retrieval quality** | [ ] | Select a semantic embedding baseline compatible with approved deployment modes; add relevance-judged, distractor-heavy eval questions; measure recall@k, MRR, and nDCG; evaluate chunking, hybrid lexical/vector retrieval, reranking, and query expansion against the baseline; add groundedness/faithfulness checks for generated answers; adopt techniques only when they improve measured quality within latency and cost budgets. |
