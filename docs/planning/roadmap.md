...
| H6 | **Dashboard read-model hardening** | ✅ | `payment_read_models` powers dashboard list/search when Kafka projections are enabled. Delivered: `payment_method_type` column (V62 migration) populated via backfill (unions 64 physical `payment_requests_*` shards) and Kafka projection consumer (extracts from `attempts[0]`); server-side `?method=` filter on both projection and live-query paths; `/projection-health` endpoint + dashboard yellow lag banner; on-demand admin backfill endpoint. Observability: `ReadModelMetrics` Micrometer component exposes `payment_read_model_table_bytes`, `_index_bytes`, `_total_bytes` gauges; Grafana "Read Model Observability (H6)" row with size, scan-ratio, row-count, dead-tuple, and payment-list p95/p99 latency panels; alert rules for seq-scan anomalies and dead-tuple ratio. Keep OpenSearch as an optional future adapter only if dashboard search outgrows Postgres. |
...
| H8 | **Capacity proof (publishable)** | ✅ | Rigorous, reproducible capacity proof for the charge-intent core API. **Rig:** nginx ALB + 2 backend nodes (1c/2g each) + isolated Postgres (2c/4g), resource-capped via `docker-compose.capacity.yml`. **Method:** open-model k6 (`bench/k6/capacity.js`, arrival-rate — no coordinated omission), log-normal connector latency (p50 120ms / p99 380ms + ~8.6% decline rate) via Mason Simulator, dedicated `application-capacity.yml` profile + isolated `paygateway_capacity` DB. **Results (documented in `bench/RESULTS.md`):** postgres-only ceiling ~190 charges/s, infra (Redis+Kafka) ceiling ~250 charges/s (+30%); bottleneck is DB connections held across the connector call (Little's Law: pool 40 ÷ ~0.42s hold ≈ 190/s); PG and backend CPU both have headroom. Scale levers documented: PgBouncer (transaction pooling) or eliminate connection hold across `dispatcher.charge()`. Methodology + disclosed caveats in `bench/CAPACITY.md`. **Caveat:** runs were co-located on the M1; the postgres-only vs infra delta is trustworthy under identical conditions; absolute off-box numbers (wired M2) remain an optional follow-up. |
...
| MR4 | **Ledger integration and reconciliation** | ✅ | `RailSettlementEvent` published to Kafka on settlement, VA `SettlementEventConsumer` extended for rail events, seeded rail accounts, `GET /internal/rail/reconciliation/exceptions` surfaces unmatched/amount-mismatch/late-response exceptions (tenant-scoped, requires internal token). |
| MR5 | **VA Account APIs + gateway-rail bridge** | ✅ | `POST /internal/va/accounts` and `GET /v1/va/accounts/**` (ROLE_INTERNAL) for account management; `RailServiceClient` wires `MasonSimulatorPaymentProviderService` to real ISO 8583 path; `RailPaymentResolvedConsumer` Kafka listener transitions PROCESSING intents to FAILED/SUCCEEDED after reversal resolution; `ChargeResult.pendingAsyncResolution` field carries UNKNOWN state through the intent lifecycle. |
...
| PPC9 | **Fee schedules and economics** | [~] | Reusable `fee-engine` module FE1-FE2 is implemented; FE3-FE4 add prepaid fee schedule persistence plus immutable assessment snapshots; FE5 posts card creation and matched clearing fees from merchant wallet to platform fee income; FE7 adds `Issuing -> Fees` preview/admin tooling; FE6 adds gateway-owned assess-only `PAYMENT_CONFIRM` snapshots. Gateway fee admin tooling, refund/capture contexts, and money-moving effects remain. See [reusable fee engine plan](reusable-fee-engine-plan.md). |
