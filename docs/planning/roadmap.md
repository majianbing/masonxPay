...
| PPC9 | **Fee schedules and economics** | [~] | Reusable `fee-engine` module FE1-FE2 is implemented; FE3-FE4 add prepaid fee schedule persistence plus immutable assessment snapshots; FE5 now posts card creation fees from merchant wallet to platform fee income with stable event idempotency. Clearing/settlement hooks and gateway-service adoption remain. See [reusable fee engine plan](reusable-fee-engine-plan.md). |
