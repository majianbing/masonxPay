...
| H8 | **Capacity proof (publishable)** | [~] | Rigorous, reproducible capacity proof for the charge-intent core API. **Target:** 100 charges/s sustained, 1000/s peak, gate confirm p99 ≤ 500ms + system-error ≤ 0.1%. **Rig:** two-Mac wired (M1 SUT / Air M2 off-box k6), nginx ALB + 2 backend nodes + isolated Postgres, resource-capped via `docker-compose.capacity.yml`. **Method:** open-model k6 (`bench/k6/capacity.js`, arrival-rate — no coordinated omission), log-normal connector latency (p50 120 / p99 380 + 0.7% faults) via extended Mason Simulator, dedicated `application-capacity.yml` profile + isolated `paygateway_capacity` DB for clean teardown, both-topology comparison (postgres_only vs infra). Methodology + disclosed caveats in `bench/CAPACITY.md`. **Delivered:** harness + profile + docs. **Pending:** headline soak/spike numbers + bottleneck writeup. |
