...
| PPC9 | **Fee schedules and economics** | [~] | Reusable `fee-engine` module FE1-FE2 is implemented: Aviator expression matching, fixed/percentage calculation, rounding metadata, schema validation, and visible/hidden totals. Prepaid persistence, snapshots, and ledger posting hooks remain. See [reusable fee engine plan](reusable-fee-engine-plan.md). |
