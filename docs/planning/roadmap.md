...
| PPC9 | **Fee schedules and economics** | [~] | Current implementation milestone is merge-ready: reusable `fee-engine` module FE1-FE2, prepaid fee persistence/snapshots FE3-FE4, card creation and matched clearing fee posting FE5, `Issuing -> Fees` preview/admin tooling FE7, and gateway-owned assess-only `PAYMENT_CONFIRM` snapshots FE6. Gateway fee admin tooling, refund/capture contexts, and money-moving gateway effects remain deferred. See [reusable fee engine plan](reusable-fee-engine-plan.md). |
