...
ALTER TYPE ledger_account_type ADD VALUE IF NOT EXISTS 'PREPAID_CARD';
ALTER TYPE ledger_account_type ADD VALUE IF NOT EXISTS 'CARD_NETWORK_RECEIVABLE';
ALTER TYPE ledger_account_type ADD VALUE IF NOT EXISTS 'BANK_RAIL_RECEIVABLE';
ALTER TYPE ledger_account_type ADD VALUE IF NOT EXISTS 'SUSPENSE_UNKNOWN_TXN';

-- virtual_card: links a test card PAN token to a PREPAID_CARD LedgerAccount.
...
CREATE TABLE virtual_card (
    ...
    vcc_account_id    VARCHAR(32)               NOT NULL REFERENCES ledger_account (ledger_account_id),
    owner_account_id  VARCHAR(32)               NOT NULL REFERENCES ledger_account (ledger_account_id),
