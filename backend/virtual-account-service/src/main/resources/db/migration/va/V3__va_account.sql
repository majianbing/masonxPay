-- va_account: generic double-entry ledger account.
--
...
CREATE TYPE va_account_role   AS ENUM ('TENANT', 'PLATFORM', 'EXTERNAL');
CREATE TYPE va_account_type   AS ENUM (
...
CREATE TYPE va_account_status AS ENUM ('ACTIVE', 'FROZEN', 'CLOSED');
CREATE TYPE va_mode           AS ENUM ('TEST', 'LIVE');

CREATE TABLE va_account (
    account_id      VARCHAR(32)        PRIMARY KEY,  -- snowflake: ac_{id}
    mode            va_mode            NOT NULL,
    account_role    va_account_role    NOT NULL,
    ...
    account_type    va_account_type    NOT NULL,
    ...
    frozen_balance  NUMERIC(38, 8)     NOT NULL DEFAULT 0,

    status          va_account_status  NOT NULL DEFAULT 'ACTIVE',
    created_at      TIMESTAMPTZ        NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ        NOT NULL DEFAULT now(),

    CONSTRAINT chk_tenant_scope
        CHECK (account_role != 'TENANT' OR (merchant_id IS NOT NULL)),
    CONSTRAINT chk_external_scope
        CHECK (account_role != 'EXTERNAL' OR provider_id IS NOT NULL),
    CONSTRAINT chk_balance_non_negative
        CHECK (balance >= 0),
    CONSTRAINT chk_frozen_non_negative
        CHECK (frozen_balance >= 0),
    CONSTRAINT chk_frozen_lte_balance
        CHECK (frozen_balance <= balance)
);

-- Tenant account lookups (merchant dashboard, settlement routing)
CREATE INDEX idx_va_account_tenant ON va_account (mode, merchant_id)
    WHERE account_role = 'TENANT';

-- External mirror lookups (provider reconciliation)
CREATE INDEX idx_va_account_external ON va_account (provider_id)
    WHERE account_role = 'EXTERNAL';

-- Type-specific extension tables (class-table inheritance).
-- Type-specific columns live ONLY here — never on va_account.

CREATE TABLE va_credit_line_profile (
    account_id   VARCHAR(32) PRIMARY KEY REFERENCES va_account (account_id),
